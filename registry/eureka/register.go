...
func New(eurekaUrls []string, opts ...Option) (*Registry, error) {
	...
	clientOpts := []ClientOption{WithHeartbeatInterval(r.heartbeatInterval), WithClientContext(r.ctx), WithNamespace(r.eurekaPath)}
	if r.maxRetry > 0 {
		clientOpts = append(clientOpts, WithMaxRetry(r.maxRetry))
	}
	client := NewClient(eurekaUrls, clientOpts...)
